...
def build_backtest_prediction_table(model_type):
    ...
    for dates in end_date_list:
        ...
        # for each of the top 10 correlation & inverse correlation scores for each week, train the ML model then predict
        # the following weekly close (while hiding what that data is from the model)
...
def backtesting_buy_recommendation_list(model_type):
    buy_rec_query = f'''
        ...
        select 
          ...
          , {model_type}_backtest_top_predictions_2.stock_symbol
          , start_date, keyword, time_delay, filing_type
          , current_close_price
          , predicted_price
          , (predicted_price / {model_type}_backtest_top_predictions_2.current_close_price) - 1 as predicted_price_change_percentage
          , case 
          ...
          from {model_type}_backtest_top_predictions_2
        ...
        and {model_type}_backtest_top_predictions_2.stock_symbol = weekly_stock_openings.stock_symbol
